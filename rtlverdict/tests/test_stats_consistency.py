...
# A second frozen-numbers source, same discipline as corpus_stats.json
# (generated by a script, never hand-typed) - covers results/agent_pilot.md,
# whose numbers come from a stub harness run (scripts/agent_pilot_plumbing.py),
# not the mutation corpus. Merged into the known-numbers pool for every doc,
# not just agent_pilot.md - not a per-file allowlist, still fail-loud if a
# number isn't in EITHER generated file.
...
def _load_known_numbers() -> set[int]:
    ...
    if AGENT_PILOT_REPORT_PATH.exists():
        known |= _flatten_numbers(json.loads(AGENT_PILOT_REPORT_PATH.read_text()))
    return known
